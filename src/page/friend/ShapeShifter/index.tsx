...
class ShapeShifter {
    ...
    init() {
        ...
        let item = document.getElementById("shape-shifter")
        item.classList.add("body-ready")
    }
    ...
    timeAction(fn:(index:number)=>void, intervalCount:number, max:number, reverse:boolean) {
        ...
        let currentAction = reverse ? max : 0
        fn(currentAction)
        // eslint-disable-next-line consistent-this
        let obj = this
    ...
    performComic(sequence?, delay = 2000, reverse = false) {
        ...
        this.timeAction(fn, delay, Sequence.length-1, reverse)
    }
    stopPerformComic() {
        clearInterval(this.interval)
    }
    remove() {
        let item = document.getElementById("body-shifter")
        item.classList.remove("body-ready")
    }
...
class Point {
    ...
    // 这个好像是代表的 延迟多少帧 再结束该状态 本质上就是个gap
...
export const ShapeShifterCanvas = () => {
    useEffect(() => {
        S.init()
        S.performComic()
        return () => 
